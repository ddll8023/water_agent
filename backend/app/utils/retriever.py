...
_RETRIEVE_TIMEOUT = 60  # 整体检索超时秒数


async def ensemble_retrieve(query: str, top_k: int = 10, doc_type: str | None = None):
    """双路 RRF 检索：向量检索 + BM25 加权融合（含超时保护）"""
    vector_store = get_vector_store()

    async def _do_retrieve() -> list:
    ...
    try:
        return await asyncio.wait_for(_do_retrieve(), timeout=_RETRIEVE_TIMEOUT)
    except asyncio.TimeoutError:
        logger.warning(f"ensemble_retrieve 超时({_RETRIEVE_TIMEOUT}s): query={query[:50]}")
        return []
    except Exception as e:
        logger.error(f"ensemble_retrieve 异常: {e}", exc_info=True)
        return []
