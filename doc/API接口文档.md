...
| 事件类型 | 字段               | 说明                           |
| -------- | ------------------ | ------------------------------ |
| thinking | content (string)   | 推理思考过程的逐 token 内容    |
| chunk    | content (string)   | 逐 token 流式回答内容          |
| progress | stage, message     | 处理阶段（retrieval/rerank/generate） |
| done     | session_id, message_id | 回答完成，包含会话和消息 ID     |

**事件示例**：

```
data: {"type":"progress","stage":"retrieval","message":"正在检索知识库..."}

data: {"type":"progress","stage":"rerank","message":"正在对检索结果进行排序..."}

data: {"type":"progress","stage":"generate","message":"正在生成回答..."}

data: {"type":"thinking","content":"我们需要理解用户的问题："}
...
- **描述**：删除指定对话及其所有消息。需要 Bearer Token 认证，`admin` 或 `user` 角色。
...
- **描述**：对指定对话中的某条 AI 回复进行重试，或修改用户问题后重新生成回复。将 `message_id` 位置之后的消息标记为废弃，重新执行 RAG 管道并以 SSE 流式返回新回复。需要 Bearer Token 认证，`admin` 或 `user` 角色。

| 参数       | 类型 | 位置 | 必填 | 说明                                           |
|------------|------|------|------|------------------------------------------------|
| session_id | int  | body | 是   | 对话 ID                                        |
| message_id | int  | body | 是   | 用户消息 ID，该位置之后的消息将被废弃并重新生成 |
| query      | str  | body | 是   | 用户问题（可修改后提交）                        |
...
**响应格式**：SSE 流式（同 14.1 流式对话），`chunk` 事件返回生成片段，`done` 事件返回 `{ session_id, message_id, user_message_id }`。
