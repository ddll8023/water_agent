...
| 预警详情 | GET | /api/v1/alerts/{id} | — | 预警详情(含 id, reservoir_id, handler_id, title, alert_level, indicators, source_desc?, suggestion?, suggestion_status(无/生成中/已生成/已确认), notes[], status, detected_at, resolved_at) | ✅ **已有**，**新增 suggestion_status 字段** |
...
```
Node1: 取数据
  │  调用外部 API 获取最新一条监测原始数据
  │  无数据库操作，纯网络调用
  │  失败 → 跳过后续节点直接结束，记录错误信息
  ▼
Node2: 处理入库
  │  开数据库会话
  │  查询在线站点列表 + 所有指标列表
  │  遍历站点+指标：去重检查 → 写入监测记录表 → 预警规则判定 → 记录待缓存数据
  │  提交或回滚事务
  │  返回统计结果（记录数、预警数、待缓存数据）
  ▼
Node3: 缓存与日志
  │  遍历待缓存数据写入 Redis
  │  插入巡检日志表记录本次执行结果
...
**Node1 取数据**
- 直接复用现有的外部 API 调用函数，不做任何改动
- 入参：无（API 地址和参数已在配置常量中定义）
- 产出：最新一条监测记录的原始 JSON 数据
- 异常处理：API 超时或返回空数据 → 标记执行失败，记录错误原因，跳转到写日志节点后结束

**Node2 处理入库**
- 入参：Node1 产出的原始数据
- 核心逻辑：从现有的定时采集函数主体循环中提取为一个独立处理函数，接收数据库会话和原始数据，返回处理统计结果
- 产出：处理统计（入库记录数、新增预警数、合并预警数、待缓存的数据列表、部分失败详情）
- 容错：单个站点或指标的入库处理失败不影响整体，捕获异常后继续处理后续
- 数据库会话：在节点函数内部创建和关闭

**Node3 缓存与日志**
- 入参：Node2 产出的处理统计结果
- Redis：遍历待缓存数据列表，逐个写入趋势有序集合和最新值键值对（复用现有缓存函数）
- 巡检日志：插入日志表记录本次执行的完整结果
...
| 定时采集函数的主体循环 | **抽取**为独立处理函数，Node2 调用 |
| Redis 缓存写入函数 | **不动**，Node3 直接调用 |
| 预警规则判定函数 | **不动**，Node2 内继续调用 |
| WebSocket 预警广播 | **不动**，Node2 内继续调用（保持现有实时推送） |
| 原有定时采集函数 | **删除**，APScheduler 改为调用 Patrol 工作流 |
...
| 功能 | 方法 | 路径 | 请求参数 | 响应 | 状态 |
|------|------|------|---------|------|------|
| 巡检日志列表 | GET | /api/v1/agent/patrol-logs | status?, start_time?, end_time?, page, page_size | 分页巡检日志(含 executed_at, reservoir_count, record_count, alert_count, duration_ms, status) | ❌ 待补 |
