...
| 07j | 知识库管理 | 管理员、负责人 | M4、M8 | 部分覆盖（上传+列表） |
...
当前 M4 已实现知识库文档上传和列表接口，基于 LangChain RecursiveCharacterTextSplitter 切片、DashScope Embedding 向量化、Chroma 持久化存储。文档详情/预览/删除/重新处理等接口待后续开发。智能问答对话接口（流式 SSE）待下一阶段实现。

### 涉及页面

- **知识库管理（07j）** —— 文档上传、文档列表 —— ✅ 已实现
