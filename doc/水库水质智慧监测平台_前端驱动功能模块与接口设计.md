...
当前 M4 已实现知识库文档上传、列表、详情、删除和重新处理接口，基于 PDF→RecursiveCharacterTextSplitter、Markdown→MarkdownHeaderTextSplitter 按文件类型分策略切片、DashScope Embedding 向量化、Chroma 持久化存储。智能问答对话接口（流式 SSE）已基于三类型加权融合检索 + DeepSeek-V4 重排序 + DeepSeek-V4 Flash 流式生成实现。
