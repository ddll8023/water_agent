...
| M7 报告中心 | P1 | 报告生产：日巡检/月度/季度/事件报告自动生成与审核 | 依赖 M6 |
...
| 07f | 智能问答 | 全部用户 | M4 | ✅ 已实现（SSE 流式 + RAG 知识库 + MySQL 结构化数据查询 + 对话历史管理 + 重试/修改 + 推理过程轮次展示） |
...
当前 M4 已完成从三阶段 Pipeline 到 ReAct Agent 的重构升级。核心流程：用户提问 → Agent（StateGraph + ToolNode）自主决定检索路径 → 按需调用 search_knowledge_base_tool / query_monitoring_data_tool / query_knowledge_graph_tool / check_water_standard_tool 四个工具 → 信息聚合 → LLM 流式生成带出处回答。Agent 可动态规划多步检索（如先查标准→再查数据→最后查案例），无需固定 Pipeline。同时保留知识库文档上传、列表、详情、删除和重新处理接口。
...
| **Report Generator** | Pipeline（LLM 增强工作流） | APScheduler 每天早 8 时（日报）/ 每月 1 日 9 时（月报）/ 季度首月 2 日 8 时（季报）/ REST API 手动触发 | **是** | 聚合巡检日志/趋势分析/预警数据 → LLM 生成报告 → 写入 report 表（草稿状态） |
...
Report Generator Pipeline
  ├─ 日报(每天早8时)：聚合 24h 原始数据 + patrol_analysis → LLM 生成日巡检报告 → 写入 report 表
  ├─ 月报(每月1日9时)：聚合 30 天原始数据 + 日报报告列表 → LLM 生成月度报告 → 写入 report 表
  ├─ 季报(季度首月2日8时)：聚合 90 天原始数据 + 月报报告列表 + patrol_analysis → LLM 生成季度报告 → 写入 report 表
  └─ 全部写入 report 表（status = draft，供前端预览审核）
...
M7 负责各类水质报告的自动生成与管理，包括日巡检报告、月度水质报告、季度水质报告和事件应急报告。报告内容由 Report Generator Pipeline 聚合巡检日志、趋势分析和预警数据进行生成，技术负责人审核确认后归档发布。
...
| 功能 | 方法 | 路径 | 请求参数 | 响应 | 状态 |
|------|------|------|---------|------|------|
| 报告列表 | POST | /api/v1/reports/list | page, page_size, report_type?(daily/monthly/quarterly/event), status?(draft/published/no_data), keyword? | 分页报告列表(含 id, title, report_type, status, summary, created_at, published_at) | ✅ **已实现** |
