...
| M6 Agent 智能分析引擎 | P0 | 系统大脑：2 个 Agent + 工具函数——Collector 采集与规则预警（自动采集+异常检测）、Analyst AI 趋势分析（自动）、llm_suggestion 处置建议生成（自动触发） | LangGraph、LLM |
...
M6 是系统的"大脑"，基于 LangGraph StateGraph 编排 2 个 Agent 和 1 个工具函数，将数据采集、异常检测、趋势分析、溯源推理自动化。Agent 不独立暴露为前端页面，而是通过业务模块（M3 预警中心）消费其产出。

核心设计原则：

- **Agent = 有状态工作流**：每个 Agent 是一个独立的 LangGraph StateGraph，状态通过 `TypedDict` 定义，节点函数复用现有 service 层代码
- **数据驱动协作**：Agent 之间不直接调用，通过共享数据库表隐式协作
- **事务优先**：数据库事务提交成功后，才能执行 Redis 缓存、WebSocket 广播、建议任务投递
- **失败隔离**：LLM、Neo4j、RAG 失败不影响 Collector 的 10 分钟采集任务
- **巡检日志持久化**：Collector Agent 每次执行结果写入 MySQL 巡检日志表，可追溯可查询

### 组件定义

| 组件 | 类型 | 触发方式 | 是否调 LLM | 核心闭环 |
|------|------|---------|:---------:|---------|
| **Collector Agent** | 4 节点 LangGraph | APScheduler 每 10 分钟 | 否 | 采集 API 数据 → 入库 → 规则判定 → 创建/合并预警 → WebSocket 推送 → 记录巡检日志 |
| **Analyst Agent** | 5 节点 LangGraph | APScheduler 每 6 小时 | **是** | 查询全水库 6h 数据 → 计算趋势特征 → LLM 分析 → 创建 AI 趋势预警 → 写入分析摘要 |
| **`llm_suggestion()`** | 工具函数 | 预警创建后异步触发 | **是** | Neo4j 溯源 → RAG 检索 → LLM 生成处置步骤 → 写入预警 suggestion 字段 |

### 组件协作关系

```
Collector Agent(每10分钟)
  ├─ 正常结束 → 写入巡检日志
...
Node3: process_alerts 规则预警引擎
  ...
  │  commit → 广播 WebSocket / 写 Redis / 异步触发 llm_suggestion(alert_id)
  ▼
Node4: cache_log 缓存与日志
  │  写入 Redis 趋势缓存
...
APScheduler 每 10 分钟调用 `run_collector_agent()` 入口函数。工作流没有人工审批环节，LangGraph 提供状态可追踪、失败可记录、执行可回溯。
...
```
Node1: fetch_recent_data
  │  查询所有水库最近 6h 监测记录 + 预警事件 + 上一次分析摘要
  ▼
Node2: compute_features
  │  每个(reservoir_id, indicator_id)计算完整趋势特征：
  │  delta, delta_percent, consecutive_rise/fall_count, slope, 标准差等
  ▼
Node3: ai_trend_analysis
  │  RAG 检索 + LLM 一次调用，输出按水库组织的分析结果
  │  输出格式：{reservoir_analyses: [{reservoir_id, summary, supplementary_alerts}]}
  ▼
Node4: process_alerts
  │  水库级跳过（已有未关闭 AI 预警则跳过）→ 遍历 supplementary_alerts → 直接创建 source=1 预警
  │  commit → 广播 WebSocket / 写 Redis / 异步触发 llm_suggestion(alert_id)
  ▼
Node5: write_summary
  │  每个水库写入 1 条 patrol_analysis 记录（含 supplementary_alert_ids）
```
...
调用方：Collector Agent（创建规则预警后）、Analyst Agent（创建 AI 预警后）、用户手动（`POST /api/v1/alerts/{id}/suggestion`）。

---

### 涉及页面

- **预警详情与溯源（07e）** —— Collector Agent 产出规则预警和溯源数据，`llm_suggestion()` 产出处置建议（suggestion_status 状态显示 + 确认/重新生成交互）
- **巡检报告列表与详情（07h）** —— 待 M7 模块实现
...
M7 负责各类水质报告的自动生成与管理，包括日巡检报告、季度水质报告和事件应急报告。报告内容由 Analyst Agent 提供数据统计和趋势分析，技术负责人审核确认后归档发布。
