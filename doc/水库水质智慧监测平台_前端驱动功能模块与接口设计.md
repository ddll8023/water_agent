...
| 07f | 智能问答 | 全部用户 | M4 | 待补 |
| 07g | 知识图谱可视化 | 分析师、负责人 | M5 | 待补 |
| 07h | 巡检报告列表与详情 | 全部用户 | M7 | 待补 |
| 07i | 水库站点配置管理 | 管理员 | M8 | 已覆盖 |
| 07j | 知识库管理 | 管理员、负责人 | M4、M8 | 待补 |
...
当前 M8 是接口覆盖最完整的模块，登陆、用户、角色、水库、站点、指标六组接口均已实现。后续可补充导入导出、权限树模板等辅助能力。
...
| 功能 | 方法 | 路径 | 请求参数 | 响应 | 状态 |
|------|------|------|---------|------|------|
| 创建指标 | POST | /api/indicators/create | name, code, unit?, category?(物理/化学/生物/综合), standard_limit_i~v?, is_core?(0/1) | true | ✅ |
| 指标列表 | POST | /api/indicators/list | name?, code?, category?, is_core?, page, page_size | 分页指标列表含标准限值 | ✅ |
| 指标详情 | GET | /api/indicators/{id} | — | 指标全部字段 | ✅ |
| 更新指标 | PUT | /api/indicators/{id} | name?, code?, unit?, category?, standard_limit_i~v?, is_core? | true | ✅ |
...
| 人工采样录入 | POST | — | station_id, indicator_id, value, record_time | — | ❌ 待补 |
...
预警内容的深度分析能力（溯源推理、AI 处置建议步骤化、历史相似事件匹配）依赖 M5 图谱和 M6 Agent 提供数据，当前仅存储 `source_desc` 和 `suggestion` 字符串字段，尚未支持结构化的溯源图谱和步骤化建议。WebSocket 实时推送等能力后续补充。
...
| 功能 | 方法 | 路径 | 请求参数 | 响应 | 状态 |
|------|------|------|---------|------|------|
| 预警列表 | GET | /api/v1/alerts | page, page_size, reservoir_id?, alert_level?(info/warning/critical), status?(0~3), start_time?, end_time? | 分页预警列表(含 handler_name, title, alert_level, indicators, status, detected_at, resolved_at) | ✅ |
...
当前 M4 尚未开发任何 REST 接口，包括知识库文档管理（上传/列表/删除/处理状态）和智能问答对话（流式/非流式）。所有接口待后端基于 LangChain + Chroma + Qwen-Max 实现。RAG 问答建议采用 SSE 流式输出提升交互体验。

### 涉及页面

- **智能问答（07f）** —— 对话式问答、流式输出、参考来源展示
- **知识库管理（07j）** —— 文档上传/列表/删除/处理状态

### 接口设计

| 功能 | 方法 | 路径 | 请求参数 | 响应 | 状态 |
|------|------|------|---------|------|------|
| 智能问答(流式) | POST | — | query, conversation_id? | SSE 流式 delta + 参考来源 | ❌ 待补 |
| 对话历史列表 | GET | — | page, page_size | 分页对话列表 | ❌ 待补 |
| 对话详情 | GET | — | — | 对话完整信息含消息列表 | ❌ 待补 |
| 删除对话 | DELETE | — | — | true | ❌ 待补 |
| 回答反馈 | POST | — | message_id, feedback(like/dislike) | true | ❌ 待补 |
| 知识库文档列表 | GET | — | keyword?, status?, page, page_size | 分页文档列表含处理状态 | ❌ 待补 |
| 上传文档 | POST | — | file, title?, category? | document_id + status | ❌ 待补 |
| 文档处理状态 | GET | — | — | status + progress | ❌ 待补 |
| 文档内容预览 | GET | — | — | 文档文本内容 | ❌ 待补 |
| 删除文档 | DELETE | — | — | true | ❌ 待补 |
| 重新处理文档 | POST | — | — | true | ❌ 待补 |
...
当前 M5 已初始化 Neo4j 数据库驱动，但尚未实现任何面向前端的 REST 查询接口。所有图谱能力（可视化全局图谱、节点搜索/扩展、污染溯源路径查询）待后端基于 LangChain GraphCypherQAChain 或自定义 Cypher 查询实现。预警详情中的溯源能力依赖本模块提供数据。

### 涉及页面

- **知识图谱可视化（07g）** —— 力导向图展示、节点搜索、详情面板
- **预警详情与溯源（07e）** —— 溯源图谱区域
- **水源调度建议（07l）** —— 管网拓扑图
...
| 功能 | 方法 | 路径 | 请求参数 | 响应 | 状态 |
|------|------|------|---------|------|------|
| 图谱全局概览 | GET | — | reservoir_id? | nodes[](所有实体) + edges[](所有关系) | ❌ 待补 |
| 节点搜索 | GET | — | keyword, type?(reservoir/river/pollution) | 匹配节点列表 | ❌ 待补 |
| 节点详情 | GET | — | type, id | 节点完整属性 | ❌ 待补 |
| 节点一跳扩展 | GET | — | type, id | 该节点相邻子图 | ❌ 待补 |
| 污染溯源路径 | GET | — | alert_id 或 reservoir_id + indicator_id | 溯源路径 nodes + edges | ❌ 待补 |
