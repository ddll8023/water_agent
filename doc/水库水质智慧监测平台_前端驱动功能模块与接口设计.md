...
本次新增 **自动预警检测**：定时采集数据入库后自动匹配 `alert_rule` 表规则，按比较方向+触发等级判定是否超标，去重后自动创建预警事件。默认预置 10 条全局规则覆盖 COD/氨氮/溶解氧/浊度/pH 五个核心指标。
...
| 新预警实时推送 | WS | /ws/alerts | — | 新预警事件 JSON（含 type/id/title/alert_level/status） | ✅ |
...
当前 M4 已完成知识库文档上传接口，基于 LangChain RecursiveCharacterTextSplitter 切片、DashScope Embedding 向量化、Chroma 持久化存储。文档列表/详情/预览/删除/重新处理等接口待后续开发。智能问答对话接口（流式 SSE）待下一阶段实现。

### 涉及页面

- **知识库管理（07j）** —— 文档上传 —— ✅ 已实现
- **智能问答（07f）** —— 对话式问答、流式输出、参考来源展示 —— ❌ 待下一阶段
...
| 文档列表 | GET | /api/v1/documents | keyword?, doc_type?, status?, page, page_size | 分页文档列表(含 id, title, file_name, file_size, doc_type, status, chunk_count, created_at) | ❌ 待补 |
| 文档详情 | GET | /api/v1/documents/{id} | — | 文档全部字段(含 id, title, file_name, file_size, doc_type, status, chunk_count, content, metadata, created_at, updated_at) | ❌ 待补 |
| 文档内容预览 | GET | /api/v1/documents/{id}/content | — | content(文档纯文本内容) | ❌ 待补 |
| 删除文档 | DELETE | /api/v1/documents/{id} | — | true | ❌ 待补 |
| 重新处理文档 | POST | /api/v1/documents/{id}/reprocess | — | true | ❌ 待补 |

#### 6.2 智能问答对话（下一阶段规划）

| 功能 | 方法 | 路径 | 请求参数 | 响应 | 状态 |
|------|------|------|---------|------|------|
| 智能问答(流式) | POST | /api/v1/rag/chat | query, conversation_id? | SSE 流式 delta + 参考来源 | ❌ 待补 |
| 对话历史列表 | GET | /api/v1/rag/conversations | page, page_size | 分页对话列表 | ❌ 待补 |
| 对话详情 | GET | /api/v1/rag/conversations/{id} | — | 对话完整信息含消息列表 | ❌ 待补 |
| 删除对话 | DELETE | /api/v1/rag/conversations/{id} | — | true | ❌ 待补 |
| 回答反馈 | POST | /api/v1/rag/messages/{id}/feedback | feedback(like/dislike) | true | ❌ 待补 |
