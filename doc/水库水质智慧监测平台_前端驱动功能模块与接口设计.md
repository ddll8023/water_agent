...
| M6 智能分析引擎 | P0 | 系统大脑：由 Pipeline（确定性采集）+ Agent（AI 趋势分析）+ 工具函数（处置建议）三层构成。Collector 采集管道（定时采集+规则判定）、Analyst Agent（AI 趋势分析，下一步改造为 ReAct Agent）、Report Generator Pipeline（报告生成）、llm_suggestion（处置建议生成） | LangGraph、LLM |
...
| **Analyst Agent** | Agent（LLM 增强工作流，下一步改造为 ReAct） | APScheduler 每 6 小时 | **是** | 查询全水库 6h 数据 → 计算趋势特征 → LLM 分析 → 创建 AI 趋势预警 → 写入分析摘要 |
...
Analyst Agent(每6小时，当前固定 DAG，下一步改为 ReAct Agent)
...
#### 下一步：改为 ReAct Agent 模式

当前 Analyst 是固定 DAG，执行路径在编译时确定。下一步改造为 ReAct Agent：

- **自循环**：LLM 分析数据后，自主决定是否需深入调查异常水库（查更多历史数据、查上游污染源、查知识库），而非一次走完
- **工具调用**：暴露 `query_monitoring()`、`neo4j_trace()`、`rag_retrieve()`、`compute_trend_features()` 等工具供 LLM 按需选择
- **差异化深度**：正常水库一笔带过，异常水库深入分析——更接近人类分析师的工作方式
...
| 阶段 | 目标 | 模式 | 状态 |
|------|------|------|:----:|
| 一 | Analyst → ReAct Agent | LLM 自循环 + 工具调用，对异常水库深入溯源分析 | 下一阶段实施 |
| 二 | M4 智能问答 → ReAct Agent | 用户提问后自主规划检索路径（MySQL/Neo4j/RAG 按需组合） | 后续规划 |
