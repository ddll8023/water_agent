...
| 07e | 预警详情与溯源 | 技术员、分析师、负责人 | M3、M5、M6 | 已覆盖（溯源图谱+污染源列表） |
...
预警内容的深度分析能力（溯源推理、AI 处置建议步骤化、历史相似事件匹配）依赖 M5 图谱和 M6 Agent 提供数据，当前仅存储 `source_desc` 和 `suggestion` 字符串字段，已支持结构化溯源图谱（Neo4j）和步骤化建议。
...
- **预警详情与溯源（07e）** —— 预警基础信息、超标指标、状态流转、溯源与建议（已实现）
